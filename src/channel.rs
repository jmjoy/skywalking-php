...
use tokio::{net::UnixDatagram, task};
...
impl Consumer {
    pub fn new() -> anyhow::Result<Self> {
        ...
        let receiver = receiver.lock().map_err(|_| anyhow!("Get Lock failed"))?.take().context("The RECEIVER has been taked")?;
        let receiver = UnixDatagram::from_std(receiver)
            .context("try into tokio unix datagram failed")?;
        Ok(Self(receiver))
    }
}
